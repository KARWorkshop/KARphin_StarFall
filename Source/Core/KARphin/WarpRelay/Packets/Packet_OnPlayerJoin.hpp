...
// when a player joins offically we send a packet
...
struct Packet_OnPlayerConnect
{
  ...
  std::string SCMVersion = "";  // the version control string of their client, if they got a
                                // incompatable version, reject them
  std::string clientVersion =
      "";  // the version of their client, if they got a incompatable version, reject them
...
PackageDataIntoSFMLPacket_OnPlayerConnect(const Packet_OnPlayerConnect& data)
{
  sf::Packet packet;
