...
std::vector<GeckoCode> DownloadCodes(std::string gameID, bool* succeeded)
{
  ...
  std::string endpoint
  {
    "https://github.com/KARWorkshop/KAR-Gecko-ASM/releases/download/stardust/" + gameID + ".ini"};
  ...
  *succeeded = response.has_value();
  ...
  const std::string FP = File::GetUserPath(D_GAMESETTINGS_IDX) + gameID + ".ini";
  File::WriteStringToFile(FP, ss.str());
  Common::IniFile f;
  f.Load(FP, false);
  gcodes = LoadCodes(Common::IniFile(), f);
