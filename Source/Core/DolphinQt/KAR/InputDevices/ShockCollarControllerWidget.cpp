...
ShockCollarControllerWidget::ShockCollarControllerWidget(QWidget* parent) : QWidget(parent)
{
  CreateLayout();
  ConnectWidgets();
...
void ShockCollarControllerWidget::UpdateBluetoothAvailableStatus()
{
...
void ShockCollarControllerWidget::CreateLayout()
{
  m_shockCollar_layout = new QGridLayout();
  m_shockCollar_box = new QGroupBox(tr("Shock Collars"));
  m_shockCollar_box->setLayout(m_shockCollar_layout);
 ...
 // m_wiimote_passthrough = new QRadioButton(tr("Passthrough a Bluetooth adapter"));
  ...
  m_wiimote_pt_labels[0] = new QLabel(tr("Sync Shock PIs and pair them"));
  m_wiimote_pt_labels[1] = new QLabel(tr("Reset all Shock PI pairs"));
 ...
 // m_wiimote_speaker_data = new QCheckBox(tr("Enable Speaker Data"));
  ...
  m_shockCollar_layout->setVerticalSpacing(7);
  m_shockCollar_layout->setColumnMinimumWidth(0, GetRadioButtonIndicatorWidth() -
                                                 GetLayoutHorizontalSpacing(m_shockCollar_layout));
  m_shockCollar_layout->setColumnStretch(2, 1);

  // Passthrough BT
 // m_shockCollar_layout->addWidget(m_wiimote_passthrough, m_shockCollar_layout->rowCount(), 0, 1, -1);

  int sync_row = m_shockCollar_layout->rowCount();
  m_shockCollar_layout->addWidget(m_wiimote_pt_labels[0], sync_row, 1, 1, 2);
  m_shockCollar_layout->addWidget(m_wiimote_sync, sync_row, 3);

  int reset_row = m_shockCollar_layout->rowCount();
  m_shockCollar_layout->addWidget(m_wiimote_pt_labels[1], reset_row, 1, 1, 2);
  m_shockCollar_layout->addWidget(m_wiimote_reset, reset_row, 3);

  // Emulated BT
  //m_shockCollar_layout->addWidget(m_wiimote_emu, m_shockCollar_layout->rowCount(), 0, 1, -1);

  for (size_t i = 0; i < m_wiimote_groups.size(); i++)
  {
    auto* wm_label = m_wiimote_labels[i] = new QLabel(tr("Shock PI %1").arg(i + 1));
    auto* wm_box = m_wiimote_boxes[i] = new QComboBox();
    auto* wm_button = m_wiimote_buttons[i] = new NonDefaultQPushButton(tr("Configure"));

    for (const auto& item : {tr("None"), tr("Shock PI")})
      wm_box->addItem(item);

    int wm_row = m_shockCollar_layout->rowCount();
    m_shockCollar_layout->addWidget(wm_label, wm_row, 1);
    m_shockCollar_layout->addWidget(wm_box, wm_row, 2);
    m_shockCollar_layout->addWidget(wm_button, wm_row, 3);
  }

  //m_shockCollar_layout->addWidget(m_wiimote_real_balance_board, m_shockCollar_layout->rowCount(), 1, 1, -1);
  //m_shockCollar_layout->addWidget(m_wiimote_speaker_data, m_shockCollar_layout->rowCount(), 1, 1, -1);

  //m_shockCollar_layout->addWidget(m_wiimote_ciface, m_shockCollar_layout->rowCount(), 0, 1, -1);

  int continuous_scanning_row = m_shockCollar_layout->rowCount();
  m_shockCollar_layout->addWidget(m_wiimote_continuous_scanning, continuous_scanning_row, 0, 1, 3);
  m_shockCollar_layout->addWidget(m_wiimote_refresh, continuous_scanning_row, 3);
  ...
  m_shockCollar_layout->addWidget(m_bluetooth_unavailable, m_shockCollar_layout->rowCount(), 1, 1, -1);

  auto* layout = new QVBoxLayout;
  layout->setContentsMargins(0, 0, 0, 0);
  layout->setAlignment(Qt::AlignTop);
  layout->addWidget(m_shockCollar_box);
  setLayout(layout);
}

void ShockCollarControllerWidget::ConnectWidgets()
{
 ...
 // });
  ...
  //        &ShockCollarControllerWidget::SaveSettings);
 // connect(m_wiimote_speaker_data, &QCheckBox::toggled, this,
  //        &ShockCollarControllerWidget::SaveSettings);
  connect(m_wiimote_sync, &QPushButton::clicked, this,
          &ShockCollarControllerWidget::OnBluetoothPassthroughSyncPressed);
  connect(m_wiimote_reset, &QPushButton::clicked, this,
          &ShockCollarControllerWidget::OnBluetoothPassthroughResetPressed);
  connect(m_wiimote_refresh, &QPushButton::clicked, this,
          &ShockCollarControllerWidget::OnWiimoteRefreshPressed);
...
void ShockCollarControllerWidget::OnBluetoothPassthroughResetPressed()
{
...
void ShockCollarControllerWidget::OnBluetoothPassthroughSyncPressed()
{
  const auto ios = Core::System::GetInstance().GetIOS();
...
void ShockCollarControllerWidget::OnWiimoteRefreshPressed()
{
  WiimoteReal::Refresh();
}

void ShockCollarControllerWidget::OnWiimoteConfigure(size_t index)
{
...
void ShockCollarControllerWidget::LoadSettings(Core::State state)
{
  for (size_t i = 0; i < m_wiimote_groups.size(); i++)
  {
...
void ShockCollarControllerWidget::SaveSettings()
{
  {
